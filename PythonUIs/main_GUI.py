...
pushOffset = 0
pushUp = -50
pt1 = (w/2, h/2+90+pushUp)
pt2 = (w/2, h/2+30+pushUp)
pt3 = (w/2, h/2-30+pushUp)
pt4 = (w/2, h/2-90+pushUp)
hN = (w/2, h/2-190)
hS = (w/2, h/2+190)
hNE = (w/2+95, h/2-210)
hNW = (w/2-130, h/2-190)
hSE = (w/2+130, h/2+190)
hSW = (w/2-130, h/2+190)
hW1 = (w/2-130, h/2+90+pushUp)
hW2 = (w/2-130, h/2+30+pushUp)
hW3 = (w/2-130, h/2-30+pushUp)
hW4 = (w/2-130, h/2-90+pushUp)
hE1 = (w/2+130, h/2+90+pushUp)
hE2 = (w/2+130, h/2+30+pushUp)
hE3 = (w/2+130, h/2-30+pushUp)
hE4 = (w/2+130, h/2-90+pushUp)
...
def drawBase(argFrame,argToggleHomePts):
    F = cv2.rectangle(argFrame.copy(),(w/2-30, h/2-120+pushUp),(w/2+30, h/2+120+pushUp),maroon, 2)
...
class MainApp(QMainWindow):
    ...
    def capture(self):
        ...
        if self.pathOnToggle == 1:
            if len(self.path1) == 0:
                ...
                if self.ui.pathSelectSpinBox.value() == 1 and len(self.path1)>0:
                    self.frame = drawPath(self.path1, self.frame.copy())
                if self.ui.pathSelectSpinBox.value() == 2 and len(self.path2)>0:
                    self.frame = drawPath(self.path2, self.frame.copy())
                if self.ui.pathSelectSpinBox.value() == 3 and len(self.path3)>0:
                    self.frame = drawPath(self.path3, self.frame.copy())
                if self.ui.pathSelectSpinBox.value() == 4 and len(self.path4)>0:
    ...
    def checkDonesCallback(self):
        ...
        if self.pathInstrCounter == len(self.pathInstr) and self.ui.sendNextButton.isEnabled():
            ...
            if self.activePathAdj[len(self.activePathAdj)-1][0] == hS:
                pass
            else:
    ...
    def refreshSingleFrame(self):
        ...
        if len(self.detectedObjs) != 4:
            ...
            if abs(self.detectedObjs[matches[0]][0]-w/2)<10:
                self.path1 = []
                self.path1Instr = []
                self.path1InstrAdj = []
                print '*-*-* Path 1 not available, candy already cleaned up'
            if abs(self.detectedObjs[matches[2]][0]-w/2)<10:
                self.path2 = []
                self.path2Instr = []
                self.path2InstrAdj = []
                print '*-*-* Path 2 not available, candy already cleaned up'
            if abs(self.detectedObjs[matches[4]][0]-w/2)<10:
                self.path3 = []
                self.path3Instr = []
                self.path3InstrAdj = []
                print '*-*-* Path 3 not available, candy already cleaned up'
            if abs(self.detectedObjs[matches[6]][0]-w/2)<10:
                self.path4 = []
                self.path4Instr = []
                self.path4InstrAdj = []
                print '*-*-* Path 4 not available, candy already cleaned up'
        print '*-*-*Paths for objects calculated'
        #print self.path1
        #print self.path2
        #print self.path3
        #print self.path4
